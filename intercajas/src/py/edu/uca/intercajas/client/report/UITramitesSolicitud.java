...
public class UITramitesSolicitud extends UIBase  {
...
//		fin.getDatePicker().setVisibleYearCount(99);
	...
	VerticalPanel init() {
		...
		titulo = "Reporte de tramites solicitud";
		desde = new DateBox();
		desde.setFormat(new DateBox.DefaultFormat(dateFormat));
		hasta = new DateBox();
		hasta.setFormat(new DateBox.DefaultFormat(dateFormat));
...
//		table.setWidget(2, 1, caja);
		...
		obtener.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				BeneficiarioService.Util.get().reporteTramitesSolicitud(estado0i, estado1i, estado2i, desde.getTextBox().getValue(), hasta.getTextBox().getValue(), new TextCallback() {
					@Override
					public void onSuccess(Method method, String response) {
						Window.open( "servlet.gupld?show=" + "reports/" + response + "&nombreDescarga=tramitesSolicitud.pdf", "_parent", "");
					}
